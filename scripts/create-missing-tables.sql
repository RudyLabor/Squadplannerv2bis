-- ============================================================================
-- CRÉER LES TABLES MANQUANTES PHASE 1 & 2
-- ============================================================================
...
CREATE POLICY IF NOT EXISTS "View own reliability" ON reliability_scores
  FOR SELECT USING (user_id = auth.uid());

CREATE POLICY IF NOT EXISTS "View squad reliability" ON reliability_scores
  FOR SELECT USING (
    squad_id IN (
...
CREATE POLICY IF NOT EXISTS "View squad roles" ON roles
  FOR SELECT USING (
    squad_id IN (
      SELECT squad_id FROM squad_members WHERE user_id = auth.uid()
    )
...
CREATE POLICY IF NOT EXISTS "Admins manage roles" ON roles
  FOR ALL USING (
    squad_id IN (
...
CREATE POLICY IF NOT EXISTS "View own roles" ON user_roles
  FOR SELECT USING (user_id = auth.uid());

CREATE POLICY IF NOT EXISTS "View squad member roles" ON user_roles
  FOR SELECT USING (
    role_id IN (
...
CREATE POLICY IF NOT EXISTS "Anyone can view permissions" ON permissions
...
CREATE POLICY IF NOT EXISTS "View role permissions for squad" ON role_permissions
  FOR SELECT USING (
    role_id IN (
      SELECT r.id FROM roles r
      INNER JOIN squad_members sm ON sm.squad_id = r.squad_id
...
BEGIN
  ...
  RAISE NOTICE '   - reliability_scores';
  RAISE NOTICE '   - roles';
  RAISE NOTICE '   - user_roles';
  RAISE NOTICE '   - permissions (6 permissions seedées)';
